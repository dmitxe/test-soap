...
namespace backend\components\soap;
...
class SoapService extends Component
{
    ...
    //    $auth = $hash ? base64_decode($hash) . '@' : '';
        ...
        if ($hash) {
            ...
            $hash_array = explode(':', $hash_decode);
            $username = $hash_array[0];
            $password = $hash_array[1];
            $user = $this->auth($username, $password);
            if ($user === null) {
                $err_msg =  'Unauthorized: login or password invalid';
                $server = new \SoapServer($this->wsdlUrl, $this->getOptions());
                $server->fault(401, 'Unauthorized: login or password invalid');
                $is_run = false;
            } else {
                if (!\Yii::$app->user->login($user)) {
                    $err_msg =  'Unauthorized: undefined error';
                    $server = new \SoapServer($this->wsdlUrl, $this->getOptions());
                    $server->fault(401, 'Unauthorized: undefined error');
                    $is_run = false;
                };
            }
        } else {
            $err_msg =  'Unauthorized: not required params';
            $is_run = false;
        }

        if (!$is_run) {
            $server = new \SoapServer($this->wsdlUrl, $this->getOptions());
            $server->fault(401, $err_msg);
            exit(1);
        }
